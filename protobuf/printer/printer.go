...
func (p *Printer) printProto(dsc desc.Descriptor, out io.Writer) error {
	...
	sourceInfo := protobuf.CreateSourceInfoMap(fdp)
	...
	case *desc.FieldDescriptor:
		...
		if d.IsExtension() {
			fmt.Fprint(w, "extend ")
			extNameSi := sourceInfo.Get(append(path, protobuf.Field_extendeeTag))
	...
	case *desc.OneOfDescriptor:
		...
		for i := range md.GetFields() {
			elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_fieldsTag, elementIndex: i})
		}
...
func findElement(dsc desc.Descriptor) []int32 {
	...
	case *desc.MessageDescriptor:
		if pm, ok := d.GetParent().(*desc.MessageDescriptor); ok {
			return append(path, protobuf.Message_nestedMessagesTag, getMessageIndex(d, pm.GetNestedMessageTypes()))
		}

		return append(path, protobuf.File_messageTypeTag, getMessageIndex(d, d.GetFile().GetMessageTypes()))

	case *desc.FieldDescriptor:
		if d.IsExtension() {
			if pm, ok := d.GetParent().(*desc.MessageDescriptor); ok {
				return append(path, protobuf.Message_extensionsTag, getFieldIndex(d, pm.GetNestedExtensions()))
			}
			return append(path, protobuf.File_extensionsTag, getFieldIndex(d, d.GetFile().GetExtensions()))
		}

		return append(path, protobuf.Message_fieldsTag, getFieldIndex(d, d.GetOwner().GetFields()))

	case *desc.OneOfDescriptor:
		return append(path, protobuf.Message_oneOfsTag, getOneOfIndex(d, d.GetOwner().GetOneOfs()))

	case *desc.EnumDescriptor:
		if pm, ok := d.GetParent().(*desc.MessageDescriptor); ok {
			return append(path, protobuf.Message_enumsTag, getEnumIndex(d, pm.GetNestedEnumTypes()))
		}

		return append(path, protobuf.File_enumTypeTag, getEnumIndex(d, d.GetFile().GetEnumTypes()))

	case *desc.EnumValueDescriptor:
		return append(path, protobuf.Enum_valuesTag, getEnumValueIndex(d, d.GetEnum().GetValues()))

	case *desc.ServiceDescriptor:
		return append(path, protobuf.File_servicesTag, getServiceIndex(d, d.GetFile().GetServices()))

	case *desc.MethodDescriptor:
		return append(path, protobuf.Service_methodsTag, getMethodIndex(d, d.GetService().GetMethods()))
...
func (p *Printer) printFile(fd *desc.FileDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap) {
	...
	path[0] = protobuf.File_packageTag
	sourceInfo.PutIfAbsent(append(path, 0), sourceInfo.Get(path))

	path[0] = protobuf.File_syntaxTag
	...
	if fdp.Package != nil {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_packageTag, elementIndex: 0, order: -3})
	}
	for i := range fdp.GetDependency() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_dependencyTag, elementIndex: i, order: -2})
	}
	elements.addrs = append(elements.addrs, optionsAsElementAddrs(protobuf.File_optionsTag, -1, opts)...)
	for i := range fd.GetMessageTypes() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_messageTypeTag, elementIndex: i})
	}
	for i := range fd.GetEnumTypes() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_enumTypeTag, elementIndex: i})
	}
	for i := range fd.GetServices() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_servicesTag, elementIndex: i})
	}
	exts := p.computeExtensions(sourceInfo, fd.GetExtensions(), []int32{protobuf.File_extensionsTag})
	for i, extd := range fd.GetExtensions() {
		...
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.File_extensionsTag, elementIndex: i})
	}
	...
	for i, el := range elements.addrs {
		...
		case *desc.FieldDescriptor:
			extDecl := exts[d]
			p.printExtensions(extDecl, exts, elements, i, mf, w, sourceInfo, nil, protobuf.File_extensionsTag, pkgName, pkgName, 0)
...
func (p *Printer) computeExtensions(sourceInfo protobuf.SourceInfoMap, exts []*desc.FieldDescriptor, path []int32) extensions {
...
func (p *Printer) sort(elements elementAddrs, sourceInfo protobuf.SourceInfoMap, path []int32) {
	switch {
	case p.CustomSortFunction != nil:
...
func (p *Printer) printMessage(md *desc.MessageDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	si := sourceInfo.Get(path)
	p.printBlockElement(true, si, w, indent, func(w *writer, trailer func(int, bool)) {
		p.indent(w, indent)

		fmt.Fprint(w, "message ")
		nameSi := sourceInfo.Get(append(path, protobuf.Message_nameTag))
...
func (p *Printer) printMessageBody(md *desc.MessageDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	...
	skip := map[interface{}]bool{}
	maxTag := protobuf.GetMaxTag(md.GetMessageOptions().GetMessageSetWireFormat())

	elements := elementAddrs{dsc: md, opts: opts}
	elements.addrs = append(elements.addrs, optionsAsElementAddrs(protobuf.Message_optionsTag, -1, opts)...)
	for i := range md.AsDescriptorProto().GetReservedRange() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_reservedRangeTag, elementIndex: i})
	}
	for i := range md.AsDescriptorProto().GetReservedName() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_reservedNameTag, elementIndex: i})
	}
	for i := range md.AsDescriptorProto().GetExtensionRange() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_extensionRangeTag, elementIndex: i})
	}
	...
	for i := range md.GetNestedMessageTypes() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_nestedMessagesTag, elementIndex: i})
	}
	for i := range md.GetNestedEnumTypes() {
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_enumsTag, elementIndex: i})
	}
	exts := p.computeExtensions(sourceInfo, md.GetNestedExtensions(), append(path, protobuf.Message_extensionsTag))
	for i, extd := range md.GetNestedExtensions() {
		...
		elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Message_extensionsTag, elementIndex: i})
	}
	...
	for i, el := range elements.addrs {
		...
		case *desc.FieldDescriptor:
			if d.IsExtension() {
				extDecl := exts[d]
				p.printExtensions(extDecl, exts, elements, i, mf, w, sourceInfo, path, protobuf.Message_extensionsTag, pkg, scope, indent)
				// we printed all extensions in the group, so we can skip the others
				for _, fld := range extDecl.fields {
					skip[fld] = true
				}
...
func (p *Printer) printField(fld *desc.FieldDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, scope string, indent int) {
	var groupPath []int32
	...
	if group {
		...
		case *desc.MessageDescriptor:
			...
			parentTag = protobuf.Message_nestedMessagesTag
		case *desc.FileDescriptor:
			// group that is a top-level extension
			candidates = parent.GetMessageTypes()
			parentTag = protobuf.File_messageTypeTag
		}
	...
	p.printBlockElement(true, si, w, indent, func(w *writer, trailer func(int, bool)) {
		...
		if shouldEmitLabel(fld) {
			locSi := sourceInfo.Get(append(path, protobuf.Field_labelTag))
		...
		typeSi := sourceInfo.Get(append(path, protobuf.Field_typeTag))
		p.printElementString(typeSi, w, indent, p.typeString(fld, scope))

		if !group {
			nameSi := sourceInfo.Get(append(path, protobuf.Field_nameTag))
			p.printElementString(nameSi, w, indent, fld.GetName())
		}

		fmt.Fprint(w, "= ")
		numSi := sourceInfo.Get(append(path, protobuf.Field_numberTag))
		...
		if !fld.GetFile().IsProto3() && fld.AsFieldDescriptorProto().DefaultValue != nil {
			...
			opts[-protobuf.Field_defaultTag] = []option{{name: "default", val: defVal}}
		}

		jsn := fld.AsFieldDescriptorProto().GetJsonName()
		if jsn != "" && jsn != protobuf.JsonName(fld.GetName()) {
			opts[-protobuf.Field_jsonNameTag] = []option{{name: "json_name", val: jsn}}
		}

		p.printOptionsShort(fld, opts, protobuf.Field_optionsTag, w, sourceInfo, path, indent)
...
func (p *Printer) printOneOf(ood *desc.OneOfDescriptor, parentElements elementAddrs, startFieldIndex int, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, parentPath []int32, indent int, ooIndex int32) {
	oopath := append(parentPath, protobuf.Message_oneOfsTag, ooIndex)
	oosi := sourceInfo.Get(oopath)
	p.printBlockElement(true, oosi, w, indent, func(w *writer, trailer func(int, bool)) {
		p.indent(w, indent)
		fmt.Fprint(w, "oneof ")
		extNameSi := sourceInfo.Get(append(oopath, protobuf.OneOf_nameTag))
		...
		elements.addrs = append(elements.addrs, optionsAsElementAddrs(protobuf.OneOf_optionsTag, -1, opts)...)

		count := len(ood.GetChoices())
		for idx := startFieldIndex; count > 0 && idx < len(parentElements.addrs); idx++ {
			el := parentElements.addrs[idx]
			if el.elementType != protobuf.Message_fieldsTag {
				continue
			}
			if parentElements.at(el).(*desc.FieldDescriptor).GetOneOf() == ood {
				...
				elements.addrs = append(elements.addrs, elementAddr{elementType: -protobuf.Message_fieldsTag, elementIndex: el.elementIndex})
				count--
			}
		}
...
func (p *Printer) printExtensions(exts *extensionDecl, allExts extensions, parentElements elementAddrs, startFieldIndex int, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, parentPath []int32, extTag int32, pkg, scope string, indent int) {
	path := append(parentPath, extTag)
	p.printLeadingComments(exts.sourceInfo, w, indent)
	p.indent(w, indent)
	fmt.Fprint(w, "extend ")
	extNameSi := sourceInfo.Get(append(path, 0, protobuf.Field_extendeeTag))
...
func (p *Printer) printExtensionRanges(parent *desc.MessageDescriptor, ranges []*descriptor.DescriptorProto_ExtensionRange, maxTag int32, addrs []elementAddr, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, parentPath []int32, indent int) {
	p.indent(w, indent)
	...
	p.extractAndPrintOptionsShort(dsc, opts, mf, protobuf.ExtensionRange_optionsTag, w, sourceInfo, elPath, indent)

	fmt.Fprintln(w, ";")
}

func (p *Printer) printReservedRanges(ranges []reservedRange, maxVal int32, addrs []elementAddr, w *writer, sourceInfo protobuf.SourceInfoMap, parentPath []int32, indent int) {
	p.indent(w, indent)
...
func (p *Printer) printReservedNames(names []string, addrs []elementAddr, w *writer, sourceInfo protobuf.SourceInfoMap, parentPath []int32, indent int) {
	p.indent(w, indent)
	fmt.Fprint(w, "reserved ")
...
func (p *Printer) printEnum(ed *desc.EnumDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	si := sourceInfo.Get(path)
	p.printBlockElement(true, si, w, indent, func(w *writer, trailer func(int, bool)) {
		p.indent(w, indent)

		fmt.Fprint(w, "enum ")
		nameSi := sourceInfo.Get(append(path, protobuf.Enum_nameTag))
		...
		elements.addrs = append(elements.addrs, optionsAsElementAddrs(protobuf.Enum_optionsTag, -1, opts)...)
		for i := range ed.GetValues() {
			elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Enum_valuesTag, elementIndex: i})
		}
		for i := range ed.AsEnumDescriptorProto().GetReservedRange() {
			elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Enum_reservedRangeTag, elementIndex: i})
		}
		for i := range ed.AsEnumDescriptorProto().GetReservedName() {
			elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Enum_reservedNameTag, elementIndex: i})
		}
...
func (p *Printer) printEnumValue(evd *desc.EnumValueDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	si := sourceInfo.Get(path)
	p.printElement(true, si, w, indent, func(w *writer) {
		p.indent(w, indent)

		nameSi := sourceInfo.Get(append(path, protobuf.EnumVal_nameTag))
		p.printElementString(nameSi, w, indent, evd.GetName())
		fmt.Fprint(w, "= ")

		numSi := sourceInfo.Get(append(path, protobuf.EnumVal_numberTag))
		p.printElementString(numSi, w, indent, fmt.Sprintf("%d", evd.GetNumber()))

		p.extractAndPrintOptionsShort(evd, evd.GetOptions(), mf, protobuf.EnumVal_optionsTag, w, sourceInfo, path, indent)

		fmt.Fprint(w, ";")
	})
}

func (p *Printer) printService(sd *desc.ServiceDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	si := sourceInfo.Get(path)
	p.printBlockElement(true, si, w, indent, func(w *writer, trailer func(int, bool)) {
		p.indent(w, indent)

		fmt.Fprint(w, "service ")
		nameSi := sourceInfo.Get(append(path, protobuf.Service_nameTag))
		...
		elements.addrs = append(elements.addrs, optionsAsElementAddrs(protobuf.Service_optionsTag, -1, opts)...)
		for i := range sd.GetMethods() {
			elements.addrs = append(elements.addrs, elementAddr{elementType: protobuf.Service_methodsTag, elementIndex: i})
		}
...
func (p *Printer) printMethod(mtd *desc.MethodDescriptor, mf *dynamic.MessageFactory, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
	si := sourceInfo.Get(path)
	...
	p.printBlockElement(true, si, w, indent, func(w *writer, trailer func(int, bool)) {
		...
		fmt.Fprint(w, "rpc ")
		nameSi := sourceInfo.Get(append(path, protobuf.Method_nameTag))
		p.printElementString(nameSi, w, indent, mtd.GetName())

		fmt.Fprint(w, "( ")
		inSi := sourceInfo.Get(append(path, protobuf.Method_inputTag))
		...
		outSi := sourceInfo.Get(append(path, protobuf.Method_outputTag))
		...
		if len(opts) > 0 {
			...
			elements.addrs = optionsAsElementAddrs(protobuf.Method_optionsTag, 0, opts)
			p.sort(elements, sourceInfo, path)
...
func (p *Printer) printOptionsLong(opts []option, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
...
func (p *Printer) extractAndPrintOptionsShort(dsc interface{}, optsMsg proto.Message, mf *dynamic.MessageFactory, optsTag int32, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
...
func (p *Printer) printOptionsShort(dsc interface{}, opts map[int32][]option, optsTag int32, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int) {
...
func (p *Printer) printOptionElementsShort(addrs elementAddrs, w *writer, sourceInfo protobuf.SourceInfoMap, path []int32, indent int, expand bool) {
	if expand {
		fmt.Fprintln(w, "[")
		indent++
	} else {
...
var edges = map[edgeKind]map[int32]edgeKind{
	edgeKindFile: {
		protobuf.File_optionsTag:     edgeKindOption,
		protobuf.File_messageTypeTag: edgeKindMessage,
		protobuf.File_enumTypeTag:    edgeKindEnum,
		protobuf.File_extensionsTag:  edgeKindField,
		protobuf.File_servicesTag:    edgeKindService,
	},
	edgeKindMessage: {
		protobuf.Message_optionsTag:        edgeKindOption,
		protobuf.Message_fieldsTag:         edgeKindField,
		protobuf.Message_oneOfsTag:         edgeKindOneOf,
		protobuf.Message_nestedMessagesTag: edgeKindMessage,
		protobuf.Message_enumsTag:          edgeKindEnum,
		protobuf.Message_extensionsTag:     edgeKindField,
		protobuf.Message_extensionRangeTag: edgeKindExtensionRange,
		// TODO: reserved range tag
	},
	edgeKindField: {
		protobuf.Field_optionsTag: edgeKindOption,
	},
	edgeKindOneOf: {
		protobuf.OneOf_optionsTag: edgeKindOption,
	},
	edgeKindExtensionRange: {
		protobuf.ExtensionRange_optionsTag: edgeKindOption,
	},
	edgeKindEnum: {
		protobuf.Enum_optionsTag: edgeKindOption,
		protobuf.Enum_valuesTag:  edgeKindEnumVal,
	},
	edgeKindEnumVal: {
		protobuf.EnumVal_optionsTag: edgeKindOption,
	},
	edgeKindService: {
		protobuf.Service_optionsTag: edgeKindOption,
		protobuf.Service_methodsTag: edgeKindMethod,
	},
	edgeKindMethod: {
		protobuf.Method_optionsTag: edgeKindOption,
	},
}

func extendOptionLocations(sc protobuf.SourceInfoMap, locs []*descriptor.SourceCodeInfo_Location) {
	...
	for _, loc := range locs {
		...
		for i := 0; i+1 < len(loc.Path); i += 2 {
			...
			if nextKind == edgeKindOption {
				...
				copy(newPath, *(*[]int32)(unsafe.Pointer(&loc.Path)))
...
func (p *Printer) extractOptions(dsc desc.Descriptor, opts proto.Message, mf *dynamic.MessageFactory) (map[int32][]option, error) {
	...
	for _, fldset := range [][]*desc.FieldDescriptor{md.GetFields(), mf.GetExtensionRegistry().AllExtensionsForType(md.GetFullyQualifiedName())} {
		for _, fld := range fldset {
			if dm.HasField(fld) {
				...
				switch val := val.(type) {
				case []interface{}:
					if fld.GetNumber() == protobuf.UninterpretedOptionsTag {
	...
	if len(uninterpreted) > 0 {
		...
		options[protobuf.UninterpretedOptionsTag] = opts
	}
...
func (a elementAddrs) at(addr elementAddr) interface{} {
	...
	case *desc.EnumDescriptor:
		switch addr.elementType {
		case protobuf.Enum_optionsTag:
			return a.opts[int32(addr.elementIndex)]
		case protobuf.Enum_valuesTag:
			return dsc.GetValues()[addr.elementIndex]
		case protobuf.Enum_reservedRangeTag:
			rng := dsc.AsEnumDescriptorProto().GetReservedRange()[addr.elementIndex]
			return reservedRange{start: rng.GetStart(), end: rng.GetEnd()}
		case protobuf.Enum_reservedNameTag:
			return dsc.AsEnumDescriptorProto().GetReservedName()[addr.elementIndex]
		}
	case *desc.EnumValueDescriptor:
		if addr.elementType == protobuf.EnumVal_optionsTag {
			return a.opts[int32(addr.elementIndex)]
		}
	case *desc.ServiceDescriptor:
		switch addr.elementType {
		case protobuf.Service_optionsTag:
			return a.opts[int32(addr.elementIndex)]
		case protobuf.Service_methodsTag:
			return dsc.GetMethods()[addr.elementIndex]
		}
	case *desc.MethodDescriptor:
		if addr.elementType == protobuf.Method_optionsTag {
			return a.opts[int32(addr.elementIndex)]
		}
	case extensionRange:
		if addr.elementType == protobuf.ExtensionRange_optionsTag {
			return a.opts[int32(addr.elementIndex)]
		}
	}
...
type elementSrcOrder struct {
	elementAddrs
	sourceInfo protobuf.SourceInfoMap
	prefix     []int32
}
...
func (a elementSrcOrder) Less(i, j int) bool {
	...
	if (si == nil) != (sj == nil) {
		...
		case *desc.FileDescriptor:
			...
			if ti == protobuf.File_packageTag {
				return !swapped
			}
			if ti == protobuf.File_dependencyTag {
				if tj == protobuf.File_packageTag {
					// imports will come *after* package
					return swapped
				}
				return !swapped
			}
			if ti == protobuf.File_optionsTag {
				if tj == protobuf.File_packageTag || tj == protobuf.File_dependencyTag {
		...
		case *desc.MessageDescriptor:
			if ti == protobuf.Message_optionsTag {
				return !swapped
			}
		case *desc.EnumDescriptor:
			if ti == protobuf.Enum_optionsTag {
				return !swapped
			}
		case *desc.ServiceDescriptor:
			if ti == protobuf.Service_optionsTag {
				return !swapped
			}
		}
		return swapped
